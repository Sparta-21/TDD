...
public record OrderSearchOptionDto(
    ...
    @Schema(
        description = "검색할 사용자 ID (고객 또는 점주)",
        example = "1001"
    )
    Long userId,

    @Schema(
        description = "주문 상태 (예: PENDING, CONFIRMED, COMPLETED, CANCELLED)",
        example = "PENDING"
    )
    OrderStatus status,

    @Schema(
        description = "가게 ID (UUID 형식)",
        example = "8a2f47ab-31d2-4c3d-bb4e-2e2aafbbf403"
    )
    UUID storeId
) {
